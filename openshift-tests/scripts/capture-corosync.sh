#!/usr/bin/bash
# Stream corosync unit journal from one master. Reconnects when the node goes away
# so we keep collecting membership/token messages after the node comes back.
#
# Masters are resolved from virsh net-dhcp-leases on the hypervisor by default.
#
# Usage: capture-corosync.sh <master-0|master-1>
#
...
LOG_FILE="${LOG_DIR}/corosync-${NODE}-${TIMESTAMP}.log"
